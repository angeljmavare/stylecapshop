...
function Catalogo3ALacosteHugo() {
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 20;

    const handleClick = (id, image, precio) => {
        const mensaje = `Hola, quiero comprar de la Categoría AAA (LACOSTE Y HUGO BOSS) el modelo ${id}, imagen del producto: ${encodeURIComponent('https://stylecapshop.vercel.app/' + image)} por ${precio}$ por favor.`;
    ...
    const currentItems = ListaGorras3ALacosteHugo.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(ListaGorras3ALacosteHugo.length / itemsPerPage);
        ...
        <div className="catalogo" style={{ backgroundImage: `url(${BannerImage})` }}>
            <center>
                <div className='catalogotitle'>
                    <h1 className='text-glow'>AAA (LACOSTE Y HUGO BOSS)</h1>
                </div>
            </center>
...
export default Catalogo3ALacosteHugo;