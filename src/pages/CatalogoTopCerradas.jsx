...
function CatalogoTopCerradas() {
    ...
    const handleClick = (id, image, precio) => {
        const mensaje = `Hola, quiero comprar de la Categoría TOP QUALITY 1.1 (CERRADAS) el modelo ${id}, imagen del producto: ${encodeURIComponent('https://stylecapshop.vercel.app/' + image)} por ${precio}$ por favor.`;
    ...
    const currentItems = ListaGorrasTopCerradas.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(ListaGorrasTopCerradas.length / itemsPerPage);
        ...
        <div className="catalogo" style={{ backgroundImage: `url(${BannerImage})` }}>
            <center>
                <div className='catalogotitle'>
                    <h1 className='text-glow'>TOP QUALITY 1.1 (CERRADAS)</h1>
                </div>
            </center>
...
export default CatalogoTopCerradas;