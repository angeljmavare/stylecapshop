...
function Catalogo3ABelicas() {
    ...
    const handleClick = (id, image, precio) => {
        const mensaje = `Hola, quiero comprar de la Categoría AAA (BELICAS) el modelo ${id}, imagen del producto: ${encodeURIComponent('https://stylecapshop.vercel.app/' + image)} por ${precio}$ por favor.`;
    ...
    const currentItems = ListaGorras3ABelicas.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(ListaGorras3ABelicas.length / itemsPerPage);
        ...
        <div className="catalogo" style={{ backgroundImage: `url(${BannerImage})` }}>
            <center>
                <div className='catalogotitle'>
                    <h1 className='text-glow'>AAA (BELICAS)</h1>
                </div>
            </center>
...
export default Catalogo3ABelicas;