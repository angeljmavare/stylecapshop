...
function CatalogoTopPlanas() {
    ...
    const handleClick = (id, image, precio) => {
        const mensaje = `Hola, quiero comprar de la Categoría TOP QUALITY 1.1 (PLANAS) el modelo ${id}, imagen del producto: https://stylecapshop.vercel.app/${image} por ${precio}$ por favor.`;
    ...
    const currentItems = ListaGorrasTopPlanas.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(ListaGorrasTopPlanas.length / itemsPerPage);
        ...
        <div className="catalogo" style={{ backgroundImage: `url(${BannerImage})` }}>
            <center>
                <div className='catalogotitle'>
                    <h1 className='text-glow'>TOP QUALITY 1.1 (PLANAS)</h1>
                </div>
            </center>
...
export default CatalogoTopPlanas;