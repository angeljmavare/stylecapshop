...
import { ListaGorras3ABeisboleras } from '../helpers/ListaGorras3ABeisboleras';
import { Link } from "react-router-dom";

function Catalogo3ABeisboleras() {
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 20;

    const handleClick = (id, image, precio) => {
        const mensaje = `Hola, quiero comprar de la Categoría AAA (BEISBOLERAS) el modelo ${id}, imagen del producto: ${encodeURIComponent('https://stylecapshop.vercel.app/' + image)} por ${precio}$ por favor.`;
    ...
    const currentItems = ListaGorras3ABeisboleras.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(ListaGorras3ABeisboleras.length / itemsPerPage);
        ...
        <div className="catalogo" style={{ backgroundImage: `url(${BannerImage})` }}>
            <center>
                <div className='catalogotitle'>
                    <h1 className='text-glow'>AAA (BEISBOLERAS)</h1>
                </div>
            </center>
...
export default Catalogo3ABeisboleras;