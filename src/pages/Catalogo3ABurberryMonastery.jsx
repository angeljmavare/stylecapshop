...
import { ListaGorras3ABurberryMonastery } from '../helpers/ListaGorras3ABurberryMonastery';
import { Link } from "react-router-dom";

function Catalogo3ABurberryMonastery() {
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 20;

    const handleClick = (id, image, precio) => {
        const mensaje = `Hola, quiero comprar de la Categoría AAA (BURBERRY Y MONASTERY) el modelo ${id}, imagen del producto: ${encodeURIComponent('https://stylecapshop.vercel.app/' + image)} por ${precio}$ por favor.`;
    ...
    const currentItems = ListaGorras3ABurberryMonastery.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(ListaGorras3ABurberryMonastery.length / itemsPerPage);
        ...
        <div className="catalogo" style={{ backgroundImage: `url(${BannerImage})` }}>
            <center>
                <div className='catalogotitle'>
                    <h1 className='text-glow'>AAA (BURBERRY Y MONASTERY)</h1>
                </div>
            </center>
...
export default Catalogo3ABurberryMonastery;