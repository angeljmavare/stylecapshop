...
import { ListaGorras3AGamuzadas } from '../helpers/ListaGorras3AGamuzadas';
import { Link } from "react-router-dom";

function Catalogo3AGamuzadas() {
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 20;

    const handleClick = (id, image, precio) => {
        const mensaje = `Hola, quiero comprar de la Categoría AAA (GAMUZADAS) el modelo ${id}, imagen del producto: ${encodeURIComponent('https://stylecapshop.vercel.app/' + image)} por ${precio}$ por favor.`;
    ...
    const currentItems = ListaGorras3AGamuzadas.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(ListaGorras3AGamuzadas.length / itemsPerPage);
        ...
        <div className="catalogo" style={{ backgroundImage: `url(${BannerImage})` }}>
            <center>
                <div className='catalogotitle'>
                    <h1 className='text-glow'>AAA (GAMUZADAS)</h1>
                </div>
            </center>
...
export default Catalogo3AGamuzadas;